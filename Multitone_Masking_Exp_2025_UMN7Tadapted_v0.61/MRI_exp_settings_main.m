...
cfg.SOA         = 0.5;                                                          % 400 ms target-tone SOA, also defines window size
...
cfg.n_mskrs     = 4;                                                            % | # of possible maskers+Target Tones within a given time segment
cfg.istarget_window_index = [5,6,7,8];                                          % | which window to be selected including target
cfg.protected_window_index = 1:12;                                              % | which window to be selected as target-protected window
cfg.prtctdrgn   = 2;                                                            % | # of bands on either side of target in spectral protected region
cfg.omit_freq_len = 3;                                                          % | omit random 3 adjacent freqs for every other TR
...
key.yesKeys = KbName('g');                                                      % Forward key
key.noKeys = KbName('f');                                                       % Back key
key.i1 = KbName('a');                                                           % Response type 1 in the experiment
key.i2 = KbName('b');                                                           % Response type 2 in the experiment
...
exp.flipped_TR_index_list = [8,7,6];
