...
for block_training_count = 1:length(freq_index_list)
    ...
    if ~exit_code
        ...
        [sig_mask,sig_targ,sig_log,~,~] = prepare_stim_for_layer_fMRI(cfg,exp.targ_f_list(freq_index_list(block_training_count)),0,1,1);
            ...
            if (sum(event_code_cache(1,:)==key.MRI_trigger)>0)&&(sum(event_code_cache(2,:)==key.MRI_trigger)==0)&&(sum(event_code_cache(3,:)==key.MRI_trigger)==0)
                ...
                if isnan(TR_wait_count)
                    ...
                    if TR_sub_count==1
                        ...
                        [sig_mask,~,sig_log,~,~] = prepare_stim_for_layer_fMRI(cfg,tar_freq_list(pre_fill_count),1-catch_index_list(pre_fill_count),catch_index_list(pre_fill_count),0);
                        %[sig_mask,sig_tar,sig_log,~,~] = prepare_stim_for_layer_fMRI(cfg,tar_freq_list(pre_fill_count),0,1,1);
